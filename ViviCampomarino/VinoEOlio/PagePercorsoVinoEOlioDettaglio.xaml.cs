...
namespace ViviCampomarino.VinoEOlio {
    [XamlCompilation(XamlCompilationOptions.Compile)]
    public partial class PagePercorsoVinoEOlioDettaglio : ContentPage {
        private DataRow rowAttivita;
        public PagePercorsoVinoEOlioDettaglio(DataRow RowAttivita) {
            InitializeComponent();
        ...
        private async void TapIndirizzo_Tapped(object sender, EventArgs e) {
            try {
                ...
                Location location = new Location(Convert.ToDouble(rowAttivita["Coordinate"].ToString().Split(",")[0].Replace(".", ",")), Convert.ToDouble(rowAttivita["Coordinate"].ToString().Split(",")[1].Replace(".", ",")));
